...
public class DrawAnimation extends Applet implements ActionListener, KeyListener {
	...
	public void CreateRedPractice(int n){
		Sphere practices = new Sphere(0.1f);
		Material practicem = new Material(new Color3f(255, 0, 0),new Color3f(0, 0, 0), new Color3f(1,1,1), new Color3f(1,1,1), 64);
		...
		practicetrans.setTranslation(new Vector3d(m.redx[n][0], m.redx[n][1], m.redx[n][2]));
		practice.setTransform(practicetrans);
	}
	
	public void CreateGreenPractice(int n){
		Sphere practices = new Sphere(0.1f);
		Material practicem = new Material(new Color3f(0,255, 0),new Color3f(0, 0, 0), new Color3f(1,1,1), new Color3f(1,1,1), 64);
		Appearance practicea = new Appearance();
		practicem.setLightingEnable(true);
		practicea.setMaterial(practicem);
		practices.setAppearance(practicea);
		...
		all.addChild(practice);
		Transform3D practicetrans = new Transform3D();
		practicetrans.setTranslation(new Vector3d(m.greenx[n][0], m.greenx[n][1], m.greenx[n][2]));
		practice.setTransform(practicetrans);
	}
	
	public void CreatebulePractice(int n){
		Sphere practices = new Sphere(0.1f);
		Material practicem = new Material(new Color3f(0,0, 255),new Color3f(0, 0, 0), new Color3f(1,1,1), new Color3f(1,1,1), 64);
		Appearance practicea = new Appearance();
		practicem.setLightingEnable(true);
		practicea.setMaterial(practicem);
		practices.setAppearance(practicea);
		...
		all.addChild(practice);
		Transform3D practicetrans = new Transform3D();
		practicetrans.setTranslation(new Vector3d(m.bluex[n][0], m.bluex[n][1], m.bluex[n][2]));
		practice.setTransform(practicetrans);
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == go) {
			...
			for(int i=0;i<m.n;i++){
				new Thread(m).start();
				CreateRedPractice(m.redcount);
				CreateGreenPractice(m.greencount);
				CreatebulePractice(m.bluecount);
				m.redcount++;
				m.greencount++;
				m.bluecount++;
			}
			...
			m.red.setVelocity(m.getVelocity(m.tempred,m.tempgreen,m.tempblue));
			m.green.setPosition(m.greengbest);
			m.green.setVelocity(m.getVelocity(m.tempgreen,m.tempred,m.tempblue));
			m.blue.setPosition(m.bluegbest);
			m.blue.setVelocity(m.getVelocity(m.tempblue,m.tempgreen,m.tempred));
			timer.start();
		}
	}
